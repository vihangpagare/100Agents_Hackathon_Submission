...
async def generate_image_instagram(tool_context: ToolContext) -> Dict[str, Any]:
    """Generate image"""
    print(f"--- Tool: generate_image_instagram called ---")
    
    
    image_filename = tool_context.state.get("generated_instagram_image","")
    if image_filename :
        return {
            "action" : "generate_image_instagram",
            "message" : "Image already generated now this agent's work is finished ",
...
"""
    try:
        ...
        return {"action": "generate_image_instagram",
...
Instagram_ImageGenerator = LlmAgent(
    ...
    instruction="""
Your work is to call the generate_image_instagram tool only once!!
""",
    tools=[generate_image_instagram]
)
...
PackageDisplayer = LlmAgent(
    ...
    instruction="""
    Call display_instagram_package immediately.
""",
