...
1. Delegate to **Linkedin_Content_Drafter** sub-agent.Do not ask which topic,topic would already be chosen earlier
...
1. Delegate to **X_Tweet_Content_Drafter** sub-agent . Do not ask which topic,topic would already be chosen earlier
After transferring back DO NOT report the optimized content  to the user.


- **X Thread Creation Workflow**
When the user requests X/Twitter thread creation:
1. Delegate to **X_Thread_Content_Drafter** sub-agent.Do not ask which topic,topic would already be chosen earlier

**Posting_Agent Workflow**
When the user requests to post content:
1. Delegate to **Posting_Agent** sub-agent.Do not ask which topic,topic would already be chosen earlier


**Instagram_Content_Drafter Workflow:**
- When the user says requests instagram content creation : 
1. Delegate to **Instagram_Content_Drafter** sub-agent.Do not ask which topic,topic would already be chosen earlier.
---
...
topic_prompt = """
Generate exactly one topic idea that works for LinkedIn, Twitter/X, and Instagram based on the user’s professional profile.You also have been given recent 
news about the company here
<user_profile>
{profile}
<user_profile>
